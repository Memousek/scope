...
export const RoleDependenciesModal: React.FC<RoleDependenciesModalProps> = ({
  ...
  // Group workers by role to avoid duplicate role rows and improve UX
  const workersByRole = useMemo(() => {
    const grouped: Record<string, ActiveWorker[]> = {};
    for (const worker of activeWorkers) {
      if (!grouped[worker.role]) grouped[worker.role] = [];
      grouped[worker.role].push(worker);
    }
    return grouped;
  }, [activeWorkers]);

  const getGroupStatus = (workers: ActiveWorker[]): 'active' | 'waiting' | 'blocked' => {
    // If any blocked -> blocked; else if any active -> active; else waiting
    if (workers.some(w => w.status === 'blocked')) return 'blocked';
    if (workers.some(w => w.status === 'active')) return 'active';
    return 'waiting';
  };
          ...
          <div>
            ...
            <div className="bg-gradient-to-br from-white/90 to-white/70 dark:from-gray-800/90 dark:to-gray-700/70 backdrop-blur-sm rounded-xl p-6 border border-gray-200/50 dark:border-gray-600/30 shadow-lg">
              <div className="space-y-4">
                {Object.entries(workersByRole).map(([role, workers]) => {
                  const groupStatus = getGroupStatus(workers);
                  return (
                    <div key={role} className="relative group" title={workers.map(w => `${w.name}`).join(', ')}>
                      <div className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50/80 to-gray-100/60 dark:from-gray-700/80 dark:to-gray-600/60 rounded-xl border border-gray-200/30 dark:border-gray-600/30 hover:shadow-md transition-all duration-200">
                        <div className="flex items-center gap-4">
                          {/* Status indicator */}
                          <div className={`w-3 h-3 rounded-full ${groupStatus === 'active'
                          ...
                          {/* Role icon */}
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-sm shadow-md ${groupStatus === 'active'
                          ...
                          <div>
                            <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{role}</p>
                            <p className="text-xs text-gray-600 dark:text-gray-400">{getGroupSummary(workers)}</p>
                          </div>
                        </div>

                        {/* Status toggle buttons for the whole role group */}
                        <div className="flex items-center gap-2">
                          {getStatusOptions().map(option => (
                            <button
                              key={option.value}
                              onClick={() => workers.forEach(w => handleWorkerStatusChange(w.role, option.value as 'active' | 'waiting' | 'blocked'))}
                              className={`px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200 ${groupStatus === option.value
                                  ? 'bg-gradient-to-r shadow-lg scale-105'
                                  : 'bg-gray-100 dark:bg-gray-600 hover:bg-gray-200 dark:hover:bg-gray-500'
                                } ${option.value === 'active'
                                  ? groupStatus === option.value
